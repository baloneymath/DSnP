...
DBAppendCmd::exec(const string& option)
{
   ...
   if(doRow){
     ...
     for(size_t i=1;i < options.size();++i){
       if(options[i] == "-") value = INT_MAX;
       else if(!myStr2Int(options[i], value)){
         cerr << "Error: " << '"' << options[i] << '"' << " is not an integer!!" << endl;
         return CMD_EXEC_ERROR;
       }
       values.push_back(value);
     }
     values.resize(dbtbl.nCols(), INT_MAX);
     DBRow _D(values);
     dbtbl.addRow(_D);
   }
   //ADD COL
     ...
     values.resize(dbtbl.nRows(), INT_MAX);
     dbtbl.addCol(values);
   }
...
DBPrintCmd::exec(const string& option)
{
   ...
   if(!CmdExec::lexOptions(option, options, 0)) return CMD_EXEC_ERROR;
   if(option.size() == 0) return CmdExec::errorOption(CMD_OPT_MISSING, "");
   ...
   if(myStrNCmp("-Row", options[0], 2) == 0){
     if(options.size() == 1) return CmdExec::errorOption(CMD_OPT_MISSING, options[0]);
     else if(options.size() > 2) return CmdExec::errorOption(CMD_OPT_EXTRA, options[2]);
     if(!checkRowIdx(options[1], value))
       return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[1]);

     for(size_t i=0;i < dbtbl[value].size() - 1;++i){
       if(dbtbl[value][i] == INT_MAX) cout << '.' << ' ';
       else cout << dbtbl[value][i] << ' ';
     }
     cout << dbtbl[value][dbtbl[value].size() -1] << endl;
     return CMD_EXEC_DONE;
   }
   //CASE COL
     ...
     if(!checkColIdx(options[1], value))
       return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[1]);

     dbtbl.printCol(value);
     return CMD_EXEC_DONE;
   }
   //CASE TABLE
   ...
   //CASE DEFAULT
     ...
     if(!checkRowIdx(options[0], value)) return CMD_EXEC_ERROR;
     else{
       if(options.size() == 1) return CmdExec::errorOption(CMD_OPT_MISSING, options[0]);
       values.push_back(value);
     ...
     if(dbtbl[values[0]][values[1]] == INT_MAX) cout << "null\n";
     else cout << dbtbl[values[0]][values[1]] << endl;
     values.clear();
     return CMD_EXEC_DONE;
   }
