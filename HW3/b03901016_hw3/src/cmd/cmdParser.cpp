...
CmdParser::parseCmd(string& option)
{
   ...
   size_t pos = 0;
   string cmd;
   pos = myStrGetTok(str, cmd);

   if(getCmd(cmd) == 0) cerr << "Illegal command!! (" << cmd << ')' << endl;
   
   if(pos != string::npos) option = str.substr(pos + 1);
   else option = "";
...
{
   // TODO...
   char* now = _readBufPtr;
   //Get the string before _readBufPtr
   string token;
   vector<string> tokens;
   string tstr = str.substr(0, size_t(_readBufPtr - _readBuf));
   size_t n = myStrGetTok(tstr, token);
   while (token.size()) {
      tokens.push_back(token);
      n = myStrGetTok(tstr, token, n);
   }
   ...
   //CASE: empty or only spaces
   if(tokens.size() == 0){
     char* temp = _readBufPtr;
     cout << endl;
     for(size_t i=0;i < cmds.size();++i){
       if(i % 5 == 4) cout << setw(12) << left << cmds[i] << endl;
       else cout << setw(12) << left << cmds[i];
     }
     reprintCmd();
     _readBufPtr = temp;
   }
   //CASE: tokens == 1
     ...
     vector<string> outs;
     for(size_t i=0;i < cmds.size();++i){
       if(cmds[i].size() > tokens[0].size()){
         if(myStrNCmp(cmds[i], tokens[0], tokens[0].size()) == 0) outs.push_back(cmds[i]);
       }
     }
     //already matched
     bool is_matched = false;
     for(CmdMap::iterator it = _cmdMap.begin();it != _cmdMap.end();++it){
       string ss = it->first + it->second->getOptCmd();
       if(myStrNCmp(ss, tokens[0], ss.size()) == 0){
         cout << endl;
     ...
     string tok = tokens[0];
     //multiple match
     if(outs.size() > 1 && !is_matched && *(_readBufPtr - 1) == tok[tok.size() - 1]){
       cout << endl;
       for(size_t i=0;i < outs.size() - 1;++i){
         if(i % 5 == 4) cout << setw(12) << left << outs[i] << endl;
         else cout << setw(12) << left << outs[i];
       }
       cout << outs.back();
       reprintCmd();
       _readBufPtr = temp;
     ...
     //single match
     else if(outs.size() == 1 && !is_matched){
       if(*(_readBufPtr - 1) == tok[tok.size() - 1]){
         string s = outs[0].substr(tokens[0].size());
         for(size_t i=0;i < s.size();++i) insertChar(s[i]);
         _readBufPtr = temp + s.size();
         insertChar(' ');
       }
       else if(*(_readBufPtr - 1) != tok[tok.size() - 1]){
         for(CmdMap::iterator it = _cmdMap.begin();it != _cmdMap.end();++it){
           string ss = it->first + it->second->getOptCmd();
           if(tokens[0].size() <= ss.size()){
             if(myStrNCmp(ss, tokens[0], tokens[0].size()) == 0){
               cout << endl;
               it->second->usage(cout);
               reprintCmd();
               _readBufPtr = now;
   ...
   //CASE: tokens > 1
     ...
     for(CmdMap::iterator it = _cmdMap.begin();it != _cmdMap.end();++it){
       string ss = it->first + it->second->getOptCmd();
       if(myStrNCmp(ss, tokens[0], it->first.size()) == 0){
         cout << endl;
         it->second->usage(cout);
         reprintCmd();
         _readBufPtr = now;
...
CmdParser::getCmd(string cmd)
{
   ...
   for(CmdMap::iterator it = _cmdMap.begin();it != _cmdMap.end();++it){
     ...
     if(myStrNCmp(str, cmd, it->first.size()) == 0) e = it->second;
   }
   return e;
}
