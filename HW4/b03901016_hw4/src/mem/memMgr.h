...
public:
   ...
   void reset(size_t b = 0) {
      ...
      while (next != NULL) {
        delete _activeBlock;
        _activeBlock = next;
        next = _activeBlock->getNextBlock();
      }
      // reset _recycleList
      for (size_t i = 0; i < R_SIZE; ++i) _recycleList[i].reset();
      // set new blocksize
      if (b != _blockSize && b != 0) {
        _blockSize = b;
        delete _activeBlock;
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      // find recycle list whose _arrsize == n
      while (temp != 0) {
   ...
   T* getMem(size_t t) {
      ...
      if (!_activeBlock->getMem(t, ret)) {
        ...
        size_t _n = downtoSizeT(remain);
        if (_n > S) {
          size_t rn = getArraySize(_n);
          #ifdef MEM_DEBUG
          cout << "Recycling " << ret << " to _recycleList[" << rn << "]\n";
          #endif // MEM_DEBUG
   ...
   // Get the currently alloced number of MemBlock's
