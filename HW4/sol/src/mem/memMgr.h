...
#define toSizeT(t)      ((t + SIZE_T_1) / SIZE_T * SIZE_T)
//
...
class MemRecycleList
{
   ...
   // (Remove the comments for TODO)
   // 1. Recursively delete the recycle lists linked by _nextList
   // 2. Reset this
   void reset() {
      // TODO
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      MemRecycleList<T> *nl = &(_recycleList[m]), *rl;
      do {
         rl = nl;
         if (rl->getArrSize() == n) return rl;
         nl = rl->getNextList();
      } while (nl != 0);
      nl = new MemRecycleList<T>(n);
      rl->setNextList(nl);
      return nl;
   }
   ...
   T* getMem(size_t t) {
      ...
      if (!_activeBlock->getMem(t, ret)) {
         size_t r = downtoSizeT(_activeBlock->getRemainSize());
         if (r >= S) {
            size_t rn = getArraySize(r);
            #ifdef MEM_DEBUG
            cout << "Recycling " << ret << " to _recycleList[" << rn << "]\n";
            #endif // MEM_DEBUG
